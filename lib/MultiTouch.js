...
class MultiTouch {
    ...
    onTouchesEnded = event =>
      this._parseTerminalTouchEvent(event, 'onTouchEnded');
    onTouchesCancelled = event =>
      this._parseTerminalTouchEvent(event, 'onTouchCancelled');
    ...
    _onTouchMoved = event => {
      ...
      this.events.onTouchMoved &&
        this.events.onTouchMoved({ ...deltaProps, ...event });
    };
  
    _onTouchEnded = (event, eventName) => {
      const deltaProps = this.getDeltaProps(event);
      this.events[eventName] &&
        this.events[eventName]({ ...deltaProps, ...event });
      const { identifier } = event;
    ...
    _compareEventsAndProcess = (more, less, callback) => {
        const diff = Object.keys(more).diff(Object.keys(less));
